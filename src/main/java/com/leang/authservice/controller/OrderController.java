...
public class OrderController extends BaseResponse {
    ...
    @PostMapping
    public ResponseEntity<ApiResponse<Order>> create(@RequestBody OrderCreateRequest dto) {
        Order order = Order.builder()
                ...
                .build();
        return responseEntity(true, "Order created successfully.", HttpStatus.CREATED, orderService.create(order));
    }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<Order>> update(@PathVariable UUID id, @RequestBody Order order) {
        return responseEntity(true, "Order updated successfully.", HttpStatus.OK, orderService.update(id, order));
    }
